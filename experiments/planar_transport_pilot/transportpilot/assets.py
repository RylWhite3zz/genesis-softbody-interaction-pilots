"""Discover the packaged solid and shell assets without modifying them."""
...
def inspect_asset(asset_dir: Path) -> AssetSpec | None:
    ...
    mesh = (asset_dir / relative_mesh).resolve()
...
def get_asset(asset_id: str) -> AssetSpec:
    matches = [asset for asset in discover_assets() if asset.asset_id == asset_id]
    if len(matches) != 1:
