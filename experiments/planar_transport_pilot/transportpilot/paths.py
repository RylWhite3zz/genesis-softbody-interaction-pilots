"""Read-only inputs and project-local runtime paths."""
...
DLO_LAB_ROOT = Path(os.environ.get("DLO_LAB_ROOT", REPOSITORY_ROOT.parent / "DLO-Lab")).expanduser().resolve()
...
DIAGNOSTIC_ROOT = PROJECT_ROOT / "diagnostics"
...
def configure_writable_runtime(run_key: str) -> Path:
    safe_key = "".join(char if char.isalnum() or char in "-_" else "_" for char in run_key)
    run_cache = require_within_project(CACHE_ROOT / "runs" / safe_key)
    cache_paths = {
        ...
        "TMPDIR": run_cache / "tmp",
    }
    for name, path in cache_paths.items():
        path.mkdir(parents=True, exist_ok=True)
        os.environ[name] = str(path)
    tempfile.tempdir = str(run_cache / "tmp")
    return run_cache


def isolate_genesis_particle_sampler(run_cache: Path) -> dict[str, str]:
    ...
    runtime_source = require_within_project(run_cache / "genesis_runtime")
